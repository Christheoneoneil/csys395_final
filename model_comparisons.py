...
def metrics(mod, X_t: pd.DataFrame, y_t: pd.DataFrame, y_train: pd.DataFrame, mod_str:str)->None:
    """
    ...
    print(classification_report(y_pred=preds, y_true=y_t)) 
    jacc = jaccard_score(y_true=y_t, y_pred=preds, average = 'macro')
    print('The jaccard score is ' + str(jacc))
    balanced_accuracy = balanced_accuracy_score(y_true=y_t, y_pred=preds)
    print('The accuracy is ', balanced_accuracy)
    roc = roc_auc_score(y_true=y_t, y_score=probs, multi_class = 'ovr')
    print('The ROC AUC is ' + str(roc))
    # Plot confusion matrix
    fig, ax = plt.subplots(figsize=(8, 5))
    cmp = ConfusionMatrixDisplay(
        confusion_matrix(y_true=y_t, y_pred=preds),
...
metrics(mod=rf_model, X_t=X_test, y_t=y_test, y_train = y_train, mod_str = 'random_forest') 
...
# print(lr_model)
metrics(mod=lr_model, X_t=X_test, y_t=y_test, y_train = y_train,mod_str = 'logistic_regression')
...
# print(et_model)
metrics(mod = et_model, X_t=X_test, y_t=y_test, y_train = y_train, mod_str='extra_trees')
...
# print(mlp_model)
metrics(mod = mlp_model, X_t=X_test, y_t=y_test, y_train = y_train, mod_str= 'MLP')

from sklearn.svm import SVC
SVM_classifier = SVC(probability= True)
SVM_param_grid = {
            'class__kernel' : ['rbf', 'sigmoid'],
            'class__degree' : np.arange(3,300),
                }
SVM_model, X_test, y_test, y_train = classifier_pipeline(unengineered_dat, "alignment", classifier=SVM_classifier,
                                                cv_grid=SVM_param_grid, oversampler=SMOTE())
metrics(mod = SVM_model, X_t=X_test, y_t=y_test, y_train = y_train, mod_str= 'SVM')
